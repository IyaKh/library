...
let accountByLastName = accounts.sort((accountsA, accountsB)=>accountsA.name.last > accountsB.name.last ? 1 : -1);
...
function getAccountFullNames(accounts) {
  let fullName= accounts.map((account)=> `${account.name.first} ${account.name.last}`);
  return fullName;
  // YOUR SOLUTION HERE
